...
///////////////////////////////////////////////
// memory manager, added by ventura


MallocFunction cxmalloc = malloc;
FreeFunction cxfree = free;
ReallocFunction cxrealloc = realloc;

extern "C" DLL_EXP void CxSetMemoryManager(MallocFunction new_malloc, FreeFunction new_free, ReallocFunction new_realloc)
{
	cxmalloc = new_malloc;
	cxfree = new_free;
	cxrealloc = new_realloc;
}

void* cxcalloc(size_t count, size_t size)
{
	void* ptr = cxmalloc(count * size);
...
bool CxImage::Destroy()
{
	...
	if (info.pGhost==NULL){
		...
		if (pSelection) {cxfree(pSelection); pSelection=0;}
		if (pAlpha) {AlphaClear(); cxfree(pAlpha); pAlpha=0;}
		if (pDib) {cxfree(pDib); pDib=0;}
		return true;
	}
	return false;
}
...
void CxImage::Copy(const CxImage &src, bool copypixels, bool copyselection, bool copyalpha)
{
	...
	if (copyselection && src.pSelection){
		if (pSelection) cxfree(pSelection);
		pSelection = (uint8_t*)cxmalloc(nSize);
		memcpy(pSelection,src.pSelection,nSize);
	}
	//copy the alpha channel
	if (copyalpha && src.pAlpha){
		if (pAlpha) cxfree(pAlpha);
		pAlpha = (uint8_t*)cxmalloc(nSize);
...
//    head.biClrImportant = 0;  See SetClrImportant
	...
	pDib = cxmalloc(GetSize()); // alloc memory block to store our bitmap
    if (!pDib){
