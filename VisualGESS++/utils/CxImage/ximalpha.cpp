...
{
	if (pAlpha==NULL) {
		pAlpha = (uint8_t*)cxmalloc(head.biWidth * head.biHeight);
...
{
	if (pAlpha) { cxfree(pAlpha); pAlpha=0; }
}
////////////////////////////////////////////////////////////////////////////////
...
bool CxImage::AlphaCopy(CxImage &from)
{
	...
	if (pAlpha==NULL) pAlpha = (uint8_t*)cxmalloc(head.biWidth * head.biHeight);
...
bool CxImage::AlphaFlip()
{
	...
	uint8_t *buff = (uint8_t*)cxmalloc(head.biWidth);
...
{
	if (!pAlpha) return false;
	uint8_t* pAlpha2 = (uint8_t*)cxmalloc(head.biWidth * head.biHeight);
